...
stage('send artifacts to /etc/ansible/playbooks folder')
 ...
 { steps {sshPublisher(publishers: [sshPublisherDesc(configName: 'ansible', transfers: [sshTransfer(cleanRemote: false, excludes: '', execCommand: 'ansible-playbook deploy_ansible.yaml', execTimeout: 120000, flatten: false, makeEmptyDirs: false, noDefaultExcludes: false, patternSeparator: '[, ]+', remoteDirectory: '//etc//ansible//playbooks', remoteDirectorySDF: false, removePrefix: '', sourceFiles: 'deploy_ansible.yaml')], usePromotionTimestamp: false, useWorkspaceInPromotion: false, verbose: false)])} 
 }
